...
public interface OrderService extends OpenmrsService {
    ...
    /**
     * Save or update the given <code>order</code> in the database. If the OrderType
     * for the order is not specified, then it will be set to the one set on the
     * OrderContext if any, if none exists on the orderContext, then it will be set
     * to the one associated to the ConceptClass of the ordered concept otherwise
     * the save fails. If the CareSetting field of the order is not specified then
     * it will default to the one set on the passed in OrderContext if any otherwise
     * the save fails.
     * 
     ...
     * @should pass if the existing drug order matches the concept and drug of the
     *         DC order
     * @should fail if the existing drug order matches the concept and not drug of
     *         the DC order
     ...
     * @should default to care setting and order type defined in the order context
     *         if null
     * @should not allow changing the patient of the previous order when revising an
     *         order
     * @should not allow changing the careSetting of the previous order when
     *         revising an order
     * @should not allow changing the concept of the previous order when revising an
     *         order
     * @should not allow changing the drug of the previous drug order when revising
     *         an order
     * @should fail if concept in previous order does not match that of the revised
     *         order
     * @should fail if the existing drug order matches the concept and not drug of
     *         the revised order
     ...
     * @should fail if an active drug order for the same concept and care setting
     *         exists
     * @should pass if an active test order for the same concept and care setting
     *         exists
     * @should pass if an active order for the same concept exists in a different
     *         care setting
     * @should set Order type of Drug Order to drug order if not set and concept not
     *         mapped
     * @should set Order type of Test Order to test order if not set and concept not
     *         mapped
     * @should throw AmbiguousOrderException if an active drug order for the same
     *         drug formulation exists
     * @should pass if an active order for the same concept exists in a different
     *         care setting
     * @should fail for revision order if an active drug order for the same concept
     *         and care settings exists
     * @should pass for revision order if an active test order for the same concept
     *         and care settings exists
     ...
     * @should throw AmbiguousOrderException if disconnecting multiple active orders
     *         for the given concept
     * @should throw AmbiguousOrderException if disconnecting multiple active drug
     *         orders with the same drug
     */
    @Authorized({ PrivilegeConstants.EDIT_ORDERS, PrivilegeConstants.ADD_ORDERS })
    public Order saveOrder(Order order, OrderContext orderContext) throws APIException;

    /**
     * Save or update the given retrospective <code>order</code> in the database. If
     * the OrderType for the order is not specified, then it will be set to the one
     * set on the OrderContext if any, if none exists on the orderContext, then it
     * will be set to the one associated to the ConceptClass of the ordered concept
     * otherwise the save fails. If the CareSetting field of the order is not
     * specified then it will default to the one set on the passed in OrderContext
     * if any otherwise the save fails. Retrospective entry of orders can affect
     * downstream systems that acts on orders created. Orders cannot be stopped if
     * they are already stopped in retrospective entry.
     *
     * @param order        the Order to save
     * @param orderContext the OrderContext object
     * @return the Order that was saved
    ...
    /**
     * Completely delete an order from the database. This should not typically be
     * used unless desperately needed. Most orders should just be voided. See
    ...
    /**
     * Mark an order as voided. This functionally removes the Order from the system
     * while keeping a semblance
     * 
     ...
     * @should unset dateStopped of the previous order if the specified order is a
     *         discontinuation
     * @should unset dateStopped of the previous order if the specified order is a
     *         revision
     */
     ...
     * discontinuation order with a previousOrder that matches the specified order.
     * 
     * @param order
    ...
    /**
     * Get revision order for the given order, it is the order with the changes that
     * was created as a replacement for the specified order. In other words, it is
     * the un voided revise order with a previousOrder that matches the specified
     * order.
     * 
     * @param order
    ...
    /**
     * Gets all Orders that match the specified parameters excluding discontinuation
     * orders
     * 
     ...
     * @param includeVoided Specifies whether voided orders should be included or
     *                      not
    ...
    /**
     * Get all orders that match a variety of (nullable) criteria contained in the
     * parameter object. Each extra value for a parameter that is provided acts as
     * an "and" and will reduce the number of results returned
     *
    ...
    /**
     * Updates the fulfillerStatus of an order and the related comment and finally
     * persists it
     *
     * @param order                order whose fulfillerStatus should be changed
     * @param orderFulfillerStatus describes the new Order.FulfillerStatus the order
     *                             should be set to
     * @param fullFillerComment    is a string which describes a comment that is set
     *                             while changing the FulfillerStatus
     * @return the Order that is updated with an according fulfillerStataus and
     *         fulFillerComment
    ...
    /**
     * Gets all Order objects that use this Concept for a given patient. Orders will
     * be returned in the order in which they occurred, i.e. sorted by startDate
    ...
    /**
     * Gets the order matching the specified order number and its previous orders in
     * the ordering they occurred, i.e if this order has a previous order, fetch it
     * and if it also has a previous order then fetch it until the original one with
     * no previous order is reached
     * 
     ...
     * CareSetting, OrderType as of the specified date. Below is the criteria for
     * determining an active order:
     * 
     * <pre>
     * - Not voided
     ...
     * @param careSetting the care setting, returns all ignoring care setting if
     *                    value is null
    ...
    /**
     * Gets all non retired CareSettings if includeRetired is set to true otherwise
     * retired ones are included too
     * 
     * @param includeRetired specifies whether retired care settings should be
     *                       returned or not
     ...
     * @since 1.10
     * @param includeRetired specifies whether retired ones should be included or
     *                       not
     * @should return only non retired order frequencies if includeRetired is set to
     *         false
    ...
    /**
     * Gets all non retired order frequencies associated to concepts that match the
     * specified search phrase
     * 
     * @param searchPhrase   The string to match on
     * @param locale         The locale to match on when searching in associated
     *                       concept names
     * @param exactLocale    If false then order frequencies associated to concepts
     *                       with names in a broader locale will be matched e.g in
     *                       case en_GB is passed in then en will be matched
     * @param includeRetired Specifies if retired order frequencies that match
     ...
     * @should get frequencies with names that match the phrase and locales if exact
     *         locale is false
     * @should get frequencies with names that match the phrase and locale if exact
     *         locale is true
    ...
    public Order discontinueOrder(Order orderToDiscontinue, String reasonNonCoded, Date discontinueDate,
	    Provider orderer, Encounter encounter);
    ...
    /**
     * Get all order types, if includeRetired is set to true then retired ones will
     * be included otherwise not
     * 
     * @param includeRetired boolean flag which indicate search needs to look at
     *                       retired order types or not
    ...
    /**
     * Returns all descendants of a given order type for example Given TEST will get
     * back LAB TEST and RADIOLOGY TEST; and Given LAB TEST, will might get back
     * SEROLOGY, MICROBIOLOGY, and CHEMISTRY
     * 
     * @param orderType      the order type which needs to search for its'
     *                       dependencies
    ...
    /**
     * Gets the possible drug dosing units, i.e the set members for the concept that
     * matches the uuid specified as the value for the global property
    ...
    /**
     * Gets the possible units of dispensing, i.e the set members for the concept
    ...
    /**
     * Gets the possible units of duration, i.e the set members for the concept that
     * matches the uuid specified as the value for the global property
    ...
    /**
     * Gets the possible test specimen sources, i.e the set members for the concept
     * that matches the uuid specified as the value for the global property
    ...
    /**
     * Gets the non coded drug concept, i.e the concept that matches the uuid
